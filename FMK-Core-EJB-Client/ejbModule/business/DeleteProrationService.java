...
import fmk_core_server.AbstractMultiSentenceDBServiceConverter;
	...
	public class DeleteProrationService extends AbstractMultiSentenceDBServiceConverter {
	    ...
	    /**
	  	 ...
	  	 * @throws exception
	  	 */
		public void updateFlagProrrateado() throws Exception{
			try {
				final String QRY_UPD_PRORRATEO_FLAG = "QRY_UPD_PRORRATEO_FLAG";
				String strSQL2 = loadSQLQuery(QRY_UPD_PRORRATEO_FLAG);
				ps = conn.prepareStatement(strSQL2);
				fillService(QRY_UPD_PRORRATEO_FLAG);
				ps.executeUpdate();
			} catch (Exception e) {
				 throw new Exception (e.getMessage()); 
			}
