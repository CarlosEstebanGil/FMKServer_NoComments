...
import fmk_core_server.AbstractMultiSentenceDBServiceConverter;
	...
	@Stateless(name="HistoricOperaiService", mappedName="HistoricOperaiService")
	@LocalBean
	public class HistoricOperaiService extends AbstractMultiSentenceDBServiceConverter {
		...
		public ResultSet getOperaiSelected(String whereClause) throws Exception {
			try {					
				String strQrySql = "SELECT operai.idcliente AS idcliente, " +
									...
									"CONCAT(RIGHT(operai.numdesp, 6), '-', operai.letdesp) AS despacho, " +
									...
									"IF(LENGTH(operai.refcli) > 13, CONCAT('', TRIM(LEFT(operai.refcli, 11)), '...'), operai.refcli) AS refcli, " +									
									...
									"LEFT JOIN aduanas ON aduanas.codadu = operai.codadu " +
									"LEFT JOIN clientes  ON operai.idcliente = clientes.idcliente " +
									"WHERE 1 " +
									whereClause +
									"AND (operai.fecbaja = 0 OR ISNULL(operai.fecbaja)) " +
									"AND !(operai.tipdes = 'IDA4' OR operai.tipdes = 'RE06' OR operai.tipdes = ' RE04') " +
									"AND operai.tipdes != 'DJAI' " +
									"AND operai.fecfac >= CONCAT(YEAR(CURDATE())-2,'-01-01') " +
									"AND operai.idcliente = 65 " +
									"AND operai.tripli >= 0 " +
									"AND operai.tripli <= 9999999 " +
									"AND operai.fecini >= '20141001' " +
									"AND operai.fecini <= '20141201' " +
									"AND operai.tipdes = 'IC04' " +
									"ORDER BY operai.feccar ASC ";
